class VmixInfo {
    constructor(jsonData) {
        ...
        const keyMap = {};
        jsonData.vmix.inputs.input.forEach((i) => (keyMap[i['@attributes'].key] = Number(i['@attributes'].number)));
        this.inputs = [];
...
function renderVmixInfo(box) {
    ...
    box.querySelector('.vmixInfo').innerHTML = `
      <div class="mb-1">
        ...
        ${info.overlays
        .map((o, i) => `<span class="badge badge-success badge-outline px-1 rounded-xs"> ${'<'.repeat(i)}${o}${'>'.repeat(i)}</span>`)
        .filter(Boolean)
        .join('\n')}
      </div>
      ...
      <ol>
        ...
        return (`<li>` +
            `<span class="${input.number === active.number
...
//# sourceMappingURL=vmix-info.js.map