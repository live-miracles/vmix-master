...
function compareSlaves() {
    ...
    infos[0].inputs.forEach((input1, i) => {
        ...
        for (let j = 1; j < infos.length; j++) {
            ...
            if (input1.title.endsWith('.m4a') && input1.type === 'Video')
                input1.type = 'AudioFile';
            if (input2.title.endsWith('.m4a') && input2.type === 'Video')
                input2.type = 'AudioFile';
            // check input types
            ...
            if (['AudioFile', 'Video'].includes(input1.type) &&
...
//# sourceMappingURL=compare-vmix-info.js.map